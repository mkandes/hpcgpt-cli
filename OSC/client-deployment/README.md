...
These are the instructions for deploying the hpcGPT OpenCode CLI on an HPC cluster so users load it via Environment Modules. The files in this directory are from OSC's.
...
| `module.lua` | Lmod template that sets `PATH`, `OPENCODE_CONFIG`, and `OSC_LLM_URL` |
...
- **`OSC_LLM_URL`** — set in the modulefile (see below); referenced in config as `{env:OSC_LLM_URL}`
- **MCP servers** — Enable/Disable the MCP servers that you have deployed. Slurm and OSC Chat endpoints
...
| `OSC_LLM_URL` | Base URL for your hosted OpenAI-compatible model endpoint |
...
Loading the module sets `OPENCODE_CONFIG` and `OSC_LLM_URL` automatically. Users do not need a personal install or config export.
...
export OSC_LLM_URL=https://your-endpoint/v1
opencode
```

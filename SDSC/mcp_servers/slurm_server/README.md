...
A [Model Context Protocol](https://modelcontextprotocol.io/) server built with [FastMCP](https://github.com/jlowin/fastmcp). It exposes thin wrappers around local **Slurm** and **`expanse-client`** commands so an assistant can inspect partitions, queues, and account usage on a cluster node where those binaries exist and your user has permission to run them.

Clients connect with **Streamable HTTP** to a URL such as `http://127.0.0.1:8002/mcp`.

## Tools

| Tool | Purpose |
|------|---------|
| `expanse-client` | Runs `expanse-client user -p` and returns stdout (site-specific accounting utility; must exist on `PATH`). |
...
- **`expanse-client`** on `PATH` if you use `expanse-client` (many sites use a custom or scheduler-specific binary)
